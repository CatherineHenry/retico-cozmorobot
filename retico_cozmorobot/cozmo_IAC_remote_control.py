...
class CozmoIntelligentAdaptiveCuriosityRemoteControlModule(abstract.AbstractModule, tk.Frame):
    ...
    @staticmethod
    def output_iu():
        return IACMotorAction  # TODO change this
    ...
    def _extractor_thread(self):
        ...
        while self._extractor_thread_active:
            ...
            if isinstance(input_iu, ObjectPermanenceIU):
                ...
                # TODO: move these into a function in retico_agent
                self.agent.y = self.agent.infer(self.agent.expl_dims, self.agent.inf_dims, motor_action, input_iu.flow_uuid)
                self.agent.x = motor_action
                self.agent.m, self.agent.s = self.agent.extract_ms(self.agent.x, self.agent.y)
                ...
                ### Catherine: Change here is that I control the motor goal
                # # motor_goal = self.agent.produce(flow_uuid=flow_uuid4)
                # # self.cozmo_env.update(motor_goal, log=False)
                # remote_control_cozmo = RemoteControlCozmo(self.robot)
                # flask_helpers.run_flask(flask_app, host_port=8112)
                # motor_goal = self.robot.pose()

                # TODO: make output iu go to cozmo_remote_control which then triggers the next iu (instead of it coming from IAC module)
                output_iu.set_motor_action(motor_action=None, flow_uuid=flow_uuid4, execution_uuid=self.execution_uuid)
